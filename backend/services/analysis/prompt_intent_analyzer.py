...
Classifies user prompts into intent categories and produces a severity
score distinguishing "help me understand" from "do it for me".
...
# JSON schema for Structured Outputs (strict mode compatible)
_RESPONSE_SCHEMA = {
    "type": "object",
    "properties": {
        "explanation_request": {
...
def analyze_prompt_intent(user_prompts: List[str]) -> PromptIntentResult:
    """Classify user prompts via LLM and return structured intent results."""
    if not user_prompts:
    ...
    # Compress prompts if there are too many
    ...
    logger.info("Analyzing prompt intent for %d prompts via %s", len(user_prompts), settings.llm_model)
