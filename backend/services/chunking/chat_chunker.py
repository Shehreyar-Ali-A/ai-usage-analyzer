...
Produces two representations:
  1. **Turn-level chunks** -- each ``ChatTurnChunk`` keeps the user prompt
     together with its assistant response(s).
  2. **Assistant-output chunks** -- the assistant text from each turn is
     split into paragraph-level sub-chunks (same merge/split rules as
     assignment chunking) with a back-reference to the originating turn.
"""
...
from models import AssistantOutputChunk, ChatTurnChunk, ParsedChat
...
def chunk_chat(parsed: ParsedChat) -> tuple[List[ChatTurnChunk], List[AssistantOutputChunk]]:
    """Return turn-level chunks and paragraph-level assistant output chunks."""
...
# ---------------------------------------------------------------------------
# Internal helpers (mirror assignment_chunker merge/split logic)
# ---------------------------------------------------------------------------
