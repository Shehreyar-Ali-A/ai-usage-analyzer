"""Build the final Report from structured analysis outputs and generate Markdown."""
...
def build_report(
    ...
    increasing: List[str] = list(judgment.primary_reasons)
    decreasing: List[str] = list(judgment.counter_indicators)
...
def generate_markdown(report: Report) -> str:
    ...
    if report.factor_breakdown:
        ...
        for fb in report.factor_breakdown:
            lines.append(f"| {fb.name} | {fb.weight:.0%} | {fb.score:.2f} | {fb.explanation} |")
        lines.append("")
